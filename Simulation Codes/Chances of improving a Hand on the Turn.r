...
FD.flop=hands.data[apply(hands.data[,1:5],1,function(cards) max(table((cards-1)%/%13+1))==4),]
...
subdata=hands.data[apply(hands.data[,1:5],1,function(cards) is.element(deal(cards),c(5,9:10))),]
...
triplet.flop=hands.data[apply(hands.data[,1:5],1,function(cards) max(table((cards-1)%%13+1)))==3,]
quads.freq=sum(apply(triplet.flop[,1:6],1,function(cards) I(max(table((cards-1)%%13+1))==4)))
...
two.pairs.flop=hands.data[apply(hands.data[,1:5],1,function(cards) sum(table((cards-1)%%13+1)==2)==2),]
full.house.freq=sum(apply(two.pairs.flop[,1:6],1,function(cards) I(max(table((cards-1)%%13+1))==3)))
...
triplet.freq=sum(apply(one.pair.flop[,1:6],1,function(cards) sum(table((cards-1)%%13+1)==3)==1))
